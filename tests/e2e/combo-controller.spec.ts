...
test.describe("combo controller harness", () => {
  ...
  test("Enter でピル化し、入力欄を空にする", async ({ page }) => {
    ...
    await expect(page.getByTestId("combo-pill-label-materials")).toHaveText("豆腐");
  ...
  test("blur で確定する", async ({ page }) => {
    ...
    await expect(page.getByTestId("combo-pill-label-materials")).toHaveText("しめじ");
  ...
  test("IME中と確定直後のEnterでは確定しない", async ({ page }) => {
    ...
    await expect(page.getByTestId("combo-pill-materials")).toHaveCount(0);
    ...
    await expect(page.getByTestId("combo-pill-label-materials")).toHaveText("にんじん");
  });
  ...
  test("空入力欄のBackspace/Deleteで2段階削除になる", async ({ page }) => {
    ...
    await expect(page.getByTestId("combo-pill-materials")).toHaveClass(/pending-delete/);
  ...
  test("ピル編集のEnter保存、Escape cancel、blur保存が動く", async ({ page }) => {
    ...
    await expect(page.getByTestId("combo-pill-label-materials")).toHaveText("木綿豆腐");
    ...
    await page.locator("#outside").click();
    await expect(page.getByTestId("combo-pill-label-materials")).toHaveText("厚揚げ");
  });
});
